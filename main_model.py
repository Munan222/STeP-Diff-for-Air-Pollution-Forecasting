...
class CSDI_base(nn.Module):
    ...
    def calc_loss(
        ...
        add loss
        '''
        variance = ((predicted - observed_data) ** 2).mean()

        # variance = abs(predicted - observed_data).mean()

        # observed_predict = (current_alpha ** 0.5) ** -1 * (noisy_data - ((1.0 - current_alpha) ** 0.5 * predicted))
        # variance = ((observed_data - observed_predict) ** 2).mean()
        # variance = (abs(observed_data - observed_predict)).mean()


        
        # variance = 0
        # for b in range(B):  # Iterate over batch
        #     for l in range(L):  # Iterate over the last dimension (L)
        #         for k in range(1, K):  # Start from k=1 to avoid out-of-bounds access for k-1
        #             # Calculate the difference between predicted[b, k-1, l] and observed_data[b, k, l]
        #             # variance += (predicted[b, k-1, l] - observed_data[b, k, l]) ** 2
        #             variance += abs(predicted[b, k-1, l] - observed_data[b, k, l])


        # variance = variance / (B * L * K)

        w = 1 # -2, -1, 0, 0.25, 0.25, 1, 2, 4, 8, 16

        

        loss = abs(loss + w * variance)
